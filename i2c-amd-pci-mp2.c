			...
			union i2c_cmd_base i2c_cmd_base)
{
	...
	reg = privdata->mmio + ((i2c_cmd_base.s.bus_id == 1) ? AMD_C2P_MSG1 : AMD_C2P_MSG0);
...
{
	struct amd_mp2_dev *privdata = pci_get_drvdata(i2c_common->pci_dev);
	union i2c_cmd_base i2c_cmd_base;

	dev_dbg(ndev_dev(privdata), "%s id: %d\n", __func__, i2c_common->bus_id);
...
int amd_mp2_write(struct amd_i2c_common *i2c_common)
{
	...
	if (i2c_common->rw_cfg.length <= 32) {
		...
		memcpy_toio(privdata->mmio + AMD_C2P_MSG2,
				i2c_common->rw_cfg.buf, i2c_common->rw_cfg.length);
	} else {
		i2c_cmd_base.s.mem_type = use_dram;
		write64((u64)i2c_common->rw_cfg.dma_addr,
			privdata->mmio + AMD_C2P_MSG2);
...
static void amd_mp2_pci_do_work(struct work_struct *work)
{
	...
	if (len != i2c_common->rw_cfg.length)
		dev_err(ndev_dev(privdata), "length %d in event doesn't match buffer length %d!\n", len, i2c_common->rw_cfg.length);
	if (slave_addr != i2c_common->rw_cfg.slave_addr)
		dev_err(ndev_dev(privdata), "unexpected slave address %x (expected: %x)!\n", slave_addr, i2c_common->rw_cfg.slave_addr);
			...
			if (sts == i2c_readcomplete_event) {
				if (len <= 32)
					memcpy_fromio(i2c_common->rw_cfg.buf,
							privdata->mmio + AMD_C2P_MSG2, i2c_common->rw_cfg.length);
...
static irqreturn_t amd_mp2_irq_isr(int irq, void *dev)
{
	...
	for (bus_id = 0; bus_id < 2; bus_id++) {
		reg = privdata->mmio + ((bus_id == 0) ? AMD_P2C_MSG1 : AMD_P2C_MSG2);
		val = readl(reg);
		if (val != 0) {
					...
					struct pci_dev *pci_dev)
{
	...
	base = pci_resource_start(pci_dev, bar_index);
	size = pci_resource_len(pci_dev, bar_index);
	...
	pci_intx(pci_dev, 1);
	rc = request_irq(pci_dev->irq, amd_mp2_irq_isr, IRQF_SHARED, "mp2_irq_isr",
			 privdata);
	if (rc)
			     ...
			     const struct pci_device_id *id)
{
	...
	dev_info(&pci_dev->dev, "MP2 device found [%04x:%04x] (rev %x)\n",
		 (int)pci_dev->vendor, (int)pci_dev->device, (int)pci_dev->revision);
...
static void amd_mp2_pci_remove(struct pci_dev *pci_dev)
{
	...
	for (bus_id = 0; bus_id < 2; bus_id++)
		if (privdata->plat_common[bus_id])
			amd_i2c_unregister_cb(pci_dev, privdata->plat_common[bus_id]);
					...
					pm_message_t mesg)
{
	...
	if (ret) {
 		dev_err(ndev_dev(privdata), "pci_save_state failed = %d\n", ret);
		return ret;
	}
